...
// Engram id -> display name of the item it unlocks (items carry "e": engram id).
// Cached per data object; falls back to null when no item claims the engram.
let _engToItemName = null;
let _engToItemSrc = null;
function itemNameForEngramId(engId){
  const data = itemData();
  if (!data) return null;
  if (_engToItemSrc !== data){
    _engToItemSrc = data;
    _engToItemName = new Map();
    for (const row of Object.values(data.i || {})){
      if (row && row.e != null && row.n && !_engToItemName.has(row.e)){
        _engToItemName.set(row.e, row.n);
      }
    }
  }
  return _engToItemName.get(engId) ?? _engToItemName.get(Number(engId)) ?? null;
}
  ...
  return engramRows.map(({ id, row }) => {
    ...
    if (Array.isArray(row.pre) && row.pre.length){
      ...
      preNames = preIds.map(pid => {
        const itemName = itemNameForEngramId(pid);
...
function renderItemTabDinos(it, dropDinos, harvestDinos){
  ...
  const genDinos = giDinosForItem(it.id);
  if (genDinos.length){
    const rows = genDinos.map(d =>
    ...
    html += `
      <div class="info-section">
        <div class="iv-eyebrow">Passively Generated By</div>
        ${rows}
      </div>`;
  }
...
function renderItemPanel(itemName){
  ...
  const genDinosForTabs = giDinosForItem(it.id);
  const itemPanelTabs = [
     ...(sourceCount > 0 ? [{ id: "crates", label: `Crates (${sourceCount})` }] : []),
     ...(hasFoliage ? [{ id: "foliage", label: `Foliage (${foliageHcs.length})` }] : []),
     ...((hasDinoLoot || genDinosForTabs.length) ? [{ id: "dinos", label: `Dinos (${dropDinos.length + harvestDinos.length + genDinosForTabs.length})` }] : []),
